...
SELECT 
	...
	, data.value('(/event/data/text)[1]', 'varchar(100)')  as [Current State]
INTO #result
FROM cte_HADR


DECLARE @LastTimeOffline DATETIME
DECLARE @LastTimeOnline DATETIME

SELECT @LastTimeOffline = [TimeStamp]
FROM #result
WHERE [Current State] = 'OFFLINE'

SELECT @LastTimeOnline = [TimeStamp]
FROM #result
WHERE [Current State] = 'ONLINE'

SELECT 
	@@SERVERNAME AS [ServerName]
	, @LastTimeOnline AS [Last Time Online]
	, @LastTimeOffline AS [Last Time Offline]
	, CAST(DATEDIFF(HOUR, @LastTImeOnline, GETDATE()) AS VARCHAR(100)) + ' HOURS' AS [UP Time]
